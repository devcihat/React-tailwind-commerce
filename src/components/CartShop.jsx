                        ...
                        <div className="md:grid md:grid-cols-3 gap-2 ">
                            <div className="col-span-2 p-5">
                                <h1 className="text-xl font-medium ">Shopping Cart</h1>
                                {
                                    cart.map(product => {
                                        return (
                                            <div key={product.id} className="flex justify-between items-center mt-6 pt-6">
                                                <div className="flex items-center"> <img src={product.image} width="60" className="rounded-full " />
                                                    <div className="flex flex-col ml-3">  <div onClick={() => removeProduct(product.id)} className="cursor-pointer font-semibold hover:text-red-500 text-gray-500 text-xs">Remove</div> <span className="md:text-md font-medium">{product.name}</span> <span className="text-xs font-light text-gray-400">{product.detail}</span> </div>

                                                </div>
                                                <div className="flex justify-center items-center">
                                                    <div className="pr-8 flex ">  <span onClick={() => reduction(product.id)} className="font-semibold cursor-pointer mx-2">-</span>  <span classNameName="text-lg mr-4 ml-4">{product.count}</span> <span onClick={() => increase(product.id)} className="font-semibold cursor-pointer mx-2">+</span>

                                                    </div>
                                                    <div className="pr-8 "> <span className="text-xs font-medium">${product.price}</span> </div>
                                ...
                                <div className="flex justify-between items-center mt-6 pt-6 border-t">
                                    <div className="flex items-center"> <i className="fa fa-arrow-left text-sm pr-2"></i>
                                        <NavLink to="/shop/mobile">
                                            <button className="text-md font-medium text-blue-500 focus:outline-none">Continue Shopping</button>
                                        </NavLink>
                                    </div>
                                    <div className="flex justify-center items-end"> <span className="text-sm font-medium text-gray-400 mr-1">Subtotal:</span> <span className="text-lg font-bold text-gray-800">${total}</span> </div>
                                </div>
                            </div>
                            <div className=" p-5 bg-gray-800 rounded overflow-visible"> <span className="text-xl font-medium text-gray-100 block pb-3">Card Details</span> <span className="text-xs text-gray-400 ">Card Type</span>
                                <div className="overflow-visible flex justify-between items-center mt-2">
                                    <div className="rounded w-52 h-28 bg-red-500 py-2 px-4 relative right-10"> <span className="italic text-lg font-medium text-gray-200 underline">VISA</span>
                                ...
                                <button onClick={notify} className="h-12 mb-5 w-full bg-blue-500 rounded focus:outline-none text-white hover:bg-blue-600">Check Out</button>
                            </div>
                        </div>
                    </div>
                </div>
